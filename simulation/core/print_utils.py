...
def print_end_jobs(last_time, complete_jobs, jobs, used_workers):
    ...
    print("Simulation ended after {0:.2f} ms ({1} jobs started)".format(
        last_time, len(jobs)))
    # print("   Simulation used total of {} workers".format(len(used_workers))) 
...
def print_slowdown(slow_down_rate):
    ...
    print("|     Avg: {0:.3f}  ".format(np.mean(slow_down_rate)))
    print("|     Med: {0:.3f}  ".format(np.median(slow_down_rate)))
    print("|     Std: {0:.3f}  ".format(np.std(slow_down_rate)))
    print("+     Max: {0:.3f}  ".format(np.max(slow_down_rate)))
    print("-     Min: {0:.3f}  ".format(np.min(slow_down_rate)))
